	...
	int64_t const &state_size)
{
	...
	const auto weight_hc_h_t = at::cat({ weight_hh, weight_ch }, 1).transpose(0, 1); // convenient and efficient form for matrix multiplication
	...
	bool training)
{
	...
	AT_ASSERTM(weight_hh.size(0) % 4 == 0, "### The feature dimension(dim 0) of weight_hh is invalid; the feature size must be 4 * state_size and, therefore, a multiple of 4 ###");
	AT_ASSERTM(weight_ch.size(0) % 4 == 0, "### The feature dimension(dim 0) of weight_ch is invalid; the feature size must be 4 * state_size and, therefore, a multiple of 4 ###");
	AT_ASSERTM((weight_hh.size(0) == state_size) && (weight_ch.size(0) == state_size) && (bias.size(0) == state_size),
			   "### The feature dimensions of all the weights and bias must match, but the given dimensions were: ", weight_ih.sizes(), " ", weight_hh.sizes(), " ", weight_ch.sizes(), " ", bias.sizes(), " ###");
	AT_ASSERTM(weight_hh.size(1) == weight_ch.size(1),
			   "### There is something wrong with weight_hh and weight_ch; they must have the same dimensions, but the given dimensions are ", weight_hh.sizes(), " and ", weight_ch.sizes(), " ###");
	AT_ASSERTM(weight_ih.size(1) == input_size, "### The input feature dimension ", input_size, " does not match with the input dimension of weight_ih ", weight_ih.size(1), " ###");
	...
	at::Tensor weights)
{
	...
	if (use_cuda)
	{
		...
		AT_ASSERTM(weights.is_contiguous(), "### weights tensor is not contiguous ###");
