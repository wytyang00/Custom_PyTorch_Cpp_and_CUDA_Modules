	...
	int64_t const &state_size)
{
	...
	const auto X_size = hc_size + input_size;
	const auto sig_size = 3 * state_size;
	const auto gate_size = 4 * state_size;
	const auto batch_x_X = batch_size * X_size;
	const auto batch_x_gate = batch_size * gate_size;
	const auto batch_x_state = batch_size * state_size;

	const auto weight_hc_h_t = at::cat({ weight_hh, at::cat({ weight_ch, at::zeros(weight_ch.type(), { state_size, state_size }) }, 0) }, 1).transpose(0, 1);
	...
	const dim3 threads(32, 2);
	const dim3 blocks_gates((gate_size + threads.x - 1) / threads.x,
							(batch_size + threads.y - 1) / threads.y);
	const dim3 blocks_update((state_size + threads.x - 1) / threads.x,
							 (batch_size + threads.y - 1) / threads.y);

	size_t X_first_idx = 0;
